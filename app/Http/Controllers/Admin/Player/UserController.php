...
class UserController extends Controller{
	...
	//MANAGEMENT POSTS PLAYER
	public function post_create_player(Request $request){
        ...
        if($admin_info->role === "admin" || $admin_info->role === "agent" || $admin_info->role === "shop"){
            ...
            $html = str_replace('*msg_success*', 'Un nouveau compte player a été créé avec succès', $html);
	...
	public function deposit_retrait(Request $request){
        ...
        $validator = Validator($request->only("amount","paymentMode","player_id","routeAction"), [
        ...
        if($admin_info->role === "admin" || $admin_info->role === "agent" || $admin_info->role === "shop"){
	        ...
	        $receiver = User::where([['parent','=',$logged_id],['id','=',$request->player_id]])->first();
       		 ...
       		 if(!empty($receiver)){
                    ...
                    $transaction->save();
						...
						$html = Storage::disk('public')->get('info.txt');
						$html = str_replace('*message_error*', 'player solde insuffisant pour terminer le retrait !', $html);
						$response["d"]["action"] = 3;
						$response["d"]["status"] = 200;	
						$response["d"]["mode"] = 1;	
						$response["d"]["html"] = $html;
	...
	public function status_change(Request $request){
        ...
        $validator = Validator($request->only("activity-switch","mode","player_id","routeAction"), [
        ...
        if($admin_info->role === "admin" || $admin_info->role === "agent" || $admin_info->role === "shop"){
	        ...
	        $player = User::where([['parent','=',$logged_id],['id','=',$request->player_id]])->first();
	...
	public function reset_password_player(Request $request){
        ...
        $validator = Validator($request->only("mode","player_id","routeAction"), [
            "mode" => "required",
            "player_id" => "required|numeric",
            "routeAction" => "required"
        ]);
        ...
        if($admin_info->role === "admin" || $admin_info->role === "agent" || $admin_info->role === "shop"){
	        ...
	        $player = User::where([['parent','=',$logged_id],['id','=',$request->player_id]])->first();
       		...
       		if(!empty($player)){
   				...
   				$player->password = bcrypt($new_password);
   				$player->save();
