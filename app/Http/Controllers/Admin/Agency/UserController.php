...
class UserController extends Controller{
	...
	//MANAGEMENT POSTS AGENTS  
	public function post_create_agency(Request $request){
        ...
        if($admin_info->role === "admin"){
            ...
            $html = str_replace('*msg_success*', 'Un nouveau compte agent a été créé avec succès', $html);
	...
	public function deposit_retrait(Request $request){
        ...
        $validator = Validator($request->only("amount","paymentMode","agency_id","routeAction"), [
        ...
        if($admin_info->role === "admin"){
	        ...
	        $receiver = User::where([['parent','=',$logged_id],['id','=',$request->agency_id]])->first();
                ...
                //RETRAIT
						...
						$html = Storage::disk('public')->get('info.txt');
						$html = str_replace('*message_error*', 'Agency solde insuffisant pour terminer le retrait !', $html);
						$response["d"]["action"] = 3;
						$response["d"]["status"] = 200;	
						$response["d"]["mode"] = 1;	
						$response["d"]["html"] = $html;
	...
	public function status_change(Request $request){
        ...
        $validator = Validator($request->only("activity-switch","mode","agency_id","routeAction"), [
        ...
        if($admin_info->role === "admin"){
	        ...
	        $agency = User::where([['parent','=',$logged_id],['id','=',$request->agency_id]])->first();
	...
	public function reset_password_agency(Request $request){
        ...
        $validator = Validator($request->only("mode","agency_id","routeAction"), [
            "mode" => "required",
            "agency_id" => "required|numeric",
            "routeAction" => "required"
        ]);
        ...
        if($admin_info->role === "admin"){
	        ...
	        $agency = User::where([['parent','=',$logged_id],['id','=',$request->agency_id]])->first();
       		...
       		if(!empty($agency)){
   				...
   				$agency->password = bcrypt($new_password);
   				$agency->save();
